...
// Быстрая проверка: является ли указатель кораблём-спауэнером (Дредноут/Авианосец).
// Определяется по TypeID юнита: DREAD / DMISL / HORNET (не по SpawnManager, который
// может быть неинициализирован). Используется чтобы ограничить перехват только
// нашими кораблями (Дредноут/Авианосец).
