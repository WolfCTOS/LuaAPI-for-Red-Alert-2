...
void __cdecl Hooked_MainLoop()
{
    ...
    // (у клиентов разный FPS). Вся ЛОГИКА (Lua Update)
    // выполняется ОДИН раз на логический кадр — только когда Unsorted::CurrentFrame
    // изменился с прошлого вызова. Иначе состояние (кулдауны, цели)
...
void OnGameFrame() {
    ...
    // M16 Gate 1: one-shot diagnostic HVA frame-count scan (first logic frame,
    // i.e. after rules/voxels are loaded). Reset when the frame counter restarts,
    // so each new match rescans and the log always matches the session.
    static unsigned int s_hvaScanDoneFrame = 0;
    if (s_hvaScanDoneFrame == 0 || Unsorted::CurrentFrame < s_hvaScanDoneFrame) {
        if (Unsorted::CurrentFrame > 0) {
            LuaAPI::LogTurretHvaFrameCounts();
            s_hvaScanDoneFrame = Unsorted::CurrentFrame;
        }
    }
