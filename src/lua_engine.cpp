...
typedef const wchar_t* (__fastcall* LoadString_t)(
    const char* pLabel,
    char* pOutExtraData,
...
// ---------------------------------------------------------------------------
// Сигнатурная проверка байтовой сигнатуры пролога функции — ТОЛЬКО ЛОГГЕР.
    ...
    const char* name)
{
    ...
    if (!ReadProcessMemory(
        ...
        LUA_LOG_WARN(
            "Signature read FAILED for {} @ 0x{:08X}, read={} (continuing anyway)",
            name,
            addr,
            read);
    ...
    if (memcmp(actual, expected, sizeof(actual)) != 0) {
        LUA_LOG_WARN(
            "Signature MISMATCH for {} @ 0x{:08X}: expected=[{}] live=[{}] "
            "(non-blocking, hooking anyway)",
            name,
            addr,
            BytesToHex(expected, 8),
            BytesToHex(actual, 8));
...
const wchar_t* __fastcall Hooked_LoadString(
    const char* pLabel,
    char* pOutExtraData,
    const char* pSourceFile,
    int nLine)
...
int Engine_PrintMessage(lua_State* L) {
    ...
    if (wlen <= 0)
        return luaL_error(
            L,
            "PrintMessage: invalid UTF-8 string");
...
void PollHudMuteToggle() {
    ...
    bool down =
        (GetAsyncKeyState(0xC0) & 0x8000) != 0;
    ...
    LUA_LOG_INFO(
        "[HUD] HUD output {}",
        g_hudMuted ? "muted" : "unmuted");

    if (!g_hudMuted) {
        MessageListClass::Instance.PrintMessage(
            L"HUD output unmuted");
    }
}
...
int Engine_WeaponExists(lua_State* L) {
    ...
    __try {
        exists =
            WeaponTypeClass::Find(id) != nullptr;
    }
...
void RunInitScript(lua_State* L) {
    ...
    DWORD attrs =
        GetFileAttributesW(scriptPath.c_str());
    ...
    if (luaL_dofile(
            L,
            Narrow(scriptPath).c_str()) != LUA_OK) {
    ...
    LUA_LOG_INFO(
        "Lua engine initialized on game thread, script executed");
}
...
static void ProcessDebugInput(lua_State* L) {
    ...
    if (KeyEdge(VK_BACK, &backPrev)) {
        ...
        if (g_debugInput.mode) {
            g_debugInput.buffer.clear();
            LUA_LOG_INFO(
                "[DebugCmd] input mode ENABLED");
        } else {
            LUA_LOG_INFO(
                "[DebugCmd] input mode DISABLED");
        }
    }

    if (g_debugInput.mode) {
        // 3) Печатные A-Z, 0-9 и пробел.
        for (int vk = '0'; vk <= '9'; ++vk) {
            if (KeyEdge(vk, &charPrev[vk]) &&
                g_debugInput.buffer.size() < kDebugCmdMax) {
        ...
        if (KeyEdge(
            ...
            if (!g_debugInput.buffer.empty()) {
                ...
                LUA_LOG_INFO(
                    "[DebugCmd] empty buffer, ignored");
            }
        }
    } else {
        // Режим выключен: обновляем prev для Enter, чтобы при включении
        // зажатый Enter не дал ложный edge, и обнуляем prev печатных клавиш.
        enterPrev =
...
void InstallGameHook() {
    ...
    static const uint8_t kMainLoopSig[8] = {
        0xA0, 0xA0, 0xE9, 0xA8,
        0x00, 0x81, 0xEC, 0xB4
    };

    static const uint8_t kLoadStringSig[8] = {
        0x53, 0x56, 0x8B, 0xF2,
        0x8B, 0xD9, 0x85, 0xF6
    };
    ...
    {
        // Force PAGE_EXECUTE_READWRITE BEFORE MH_CreateHook.
        DWORD oldProtect = 0;
        ...
        if (!VirtualProtect(
            ...
            LUA_LOG_WARN(
                "VirtualProtect(MainLoop) failed (error {})",
                GetLastError());
        }
        ...
        if (status != MH_OK) {
            LUA_LOG_WARN(
                "Skipping MainLoop hook installation (degraded): "
                "MH_CreateHook returned {}",
                static_cast<int>(status));
        } else {
...

    {
        ...
        if (!VirtualProtect(
            ...
            LUA_LOG_WARN(
                "VirtualProtect(LoadString) failed (error {})",
                GetLastError());
        }
        ...
        LUA_LOG_INFO(
            "MH_CreateHook(StringTable::LoadString @ {:#x}) -> {} ({})",
            kLoadStringAddr,
            MH_StatusToString(status),
            static_cast<int>(status));
        ...
        if (status != MH_OK) {
            LUA_LOG_WARN(
                "Skipping LoadString hook installation (degraded): "
                "MH_CreateHook returned {}",
                static_cast<int>(status));
        } else {
            MH_STATUS enableStatus =
            ...
            LUA_LOG_INFO(
                "MH_EnableHook(StringTable::LoadString) -> {} ({})",
                MH_StatusToString(enableStatus),
                static_cast<int>(enableStatus));
        }
    }

    // BulletClass::Detonate hook
    LuaAPI::BulletHook::Install();

    // TechnoClass::GetPrimaryWeapon (weapon override) hook
    LuaAPI::WeaponOverride::Install();

    // EventClass::Execute_DoList hook
    EventHook::Install();
}
...
void OnGameFrame() {
    ...
    // Drain projectile impact events generated by the native BulletClass hook.
    // The native hook never calls Lua directly; events are consumed here on
    // the normal game/Lua thread.
    LuaAPI::BulletHook::ImpactEvent impacts[32];

    const int impactCount =
        LuaAPI::BulletHook::DrainImpacts(
            impacts,
            32);

    for (int i = 0; i < impactCount; ++i) {
        LUA_LOG_TRACE(
            "[BulletHook] projectile detonated at ({}, {}, {})",
            impacts[i].x,
            impacts[i].y,
            impacts[i].z);
    }

    LuaAPI::ProcessDisabledObjects(
        Unsorted::CurrentFrame);

    LuaAPI::SubTurretManager::Instance().UpdateAll();

    LUA_LOG_TRACE(
        "Frame: calling EventHook::Update()");

    LuaAPI::EventHook::Update();

    LUA_LOG_TRACE(
        "Frame: EventHook::Update() returned");

    // Fire pre-damage callbacks registered by mods (OnPreDamage).
    if (g_L && g_scriptReady) {
        int nrefs =
            static_cast<int>(
                g_preDamageCallbackRefs.size());

        for (int i = 0; i < nrefs; ++i) {
            int ref =
                g_preDamageCallbackRefs[i];
    ...
    if (g_L &&
        g_scriptReady &&
        Unsorted::CurrentFrame == 1) {

        int nrefs =
            static_cast<int>(
                g_scenarioStartCallbackRefs.size());

        for (int i = 0; i < nrefs; ++i) {
            int ref =
                g_scenarioStartCallbackRefs[i];
    ...
    if (g_L &&
        ...
        if (Unsorted::CurrentFrame > 0) {
            int nrefs =
                static_cast<int>(
                    g_unitDestroyedCallbackRefs.size());

            if (nrefs > 0) {
                for (int i = 0; i < nrefs; ++i) {
                    int ref =
                        g_unitDestroyedCallbackRefs[i];
    ...
    // Переключатель HUD-лога на клавишу ` (VK 0xC0).
    if (!g_debugInput.mode)
        PollHudMuteToggle();

    // Дебаг-консоль (dev-команды ИИ).
    ProcessDebugInput(g_L);

    // HUD-индикатор дебаг-консоли.
    if (g_debugInput.mode) {
        g_debugHudText =
            "DEBUG MODE: [" +
            g_debugInput.buffer +
            "] | ENTER=send BACKSPACE=toggle";
    } else {
        g_debugHudText.clear();
    }
...
void ResetSession() {
    ...
    // Clear native projectile tracking and pending impact events.
    LuaAPI::BulletHook::Clear();
    ...
    LUA_LOG_INFO(
        "Lua session reset: callbacks cleared, VM state reset");
}
